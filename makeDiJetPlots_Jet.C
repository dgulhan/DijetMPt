...
void addJtHistToPanel(TFile* file_p, const char* fileTag, TCanvas* canv_p, Int_t setNum, const char* jtVarIn, TLegend* leg, Int_t pos = 1, const char* cent = "50100", const char* opt = "E1")
{
  file_p->cd();
...
void makeJtVarPanel(const char* filePbPbName, const char* fileTagPbPb, const char* outName, Int_t setNum, const char* jtVarIn, const char* filePPName = "", const char* fileTagPP = "", Bool_t isHighPtTrk = false)
{
  ...
  for(Int_t centIter = 0; centIter < 4; centIter++){
    addJtHistToPanel(histPbPbFile_p, fileTagPbPb, jtVarPanel_p, setNum, jtVarIn, leg_p, centIter+1, cent[centIter]);
    
    if(strcmp(fileTagPP, "") != 0)
      addJtHistToPanel(histPPFile_p, fileTagPP, jtVarPanel_p, setNum, jtVarIn, leg_p, centIter+1, "PP", "E1 SAME");
...
void makeDiJetPlots_Jet(const char* filePbPbName, const char* fileTagPbPb, const char* outName, Bool_t montecarlo = false, const char* filePPName = "", const char* fileTagPP = "", Bool_t isPercent = false, Bool_t isHighPtTrk = false)
{
  TH1::SetDefaultSumw2();
  ...
  for(Int_t algIter = 1; algIter < jetAlgMax; algIter++){
    makeJtVarPanel(filePbPbName, fileTagPbPb, outName, algIter, "Aj", filePPName, fileTagPP, isHighPtTrk);
  }
