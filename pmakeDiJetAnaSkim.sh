if [ $# -ne 6 ]
then 
  echo "Usage: ./pmakeDiJetAnaSkim.sh <inputList> <sType> <outName> <outDir> <justJtBool> <isHITrk>"
  exit 1
fi
...
cat pmakeDiJetAnaSkim.condor | sed "s@log_flag@$now@g" | sed "s@dir_flag@$PWD/$now@g" | sed "s@arg1@$1@g" | sed "s@arg2@$2@g" | sed "s@arg3@$3@g" | sed "s@arg4@$4@g" | sed "s@arg5@$5@g" | sed "s@arg6@$6@g" | sed "s@njobs@$len@g" > $now/pmakeDiJetAnaSkim.condor
echo -=-
